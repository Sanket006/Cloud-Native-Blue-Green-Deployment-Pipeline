...
# Validate that both required Terraform outputs were retrieved
if [ -z "$ECR_URL" ]; then
    echo "Error: ECR URL not found. Ensure 'terraform apply' has been run successfully."
    exit 1
fi

if [ -z "$CLUSTER_NAME" ]; then
    echo "Error: Cluster name not found. Ensure 'terraform apply' has been run successfully."
    exit 1
fi
